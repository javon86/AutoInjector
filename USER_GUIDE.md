...
### 7e. Your output folder ✅

Everything the program produces or handles is filed into one place you can
find: **`Documents/AutoInjector/output`**, broken up by what it is:

- **`books/<book title>/`** — each bookmaking run gets its own titled folder.
- **`images/`** — images generated by Image Studio (and by the AIs).
- **`videos/`** — generated videos (when that's added).
- **`uploads/`** — a copy of every file you attach and send to the AIs, so
  there's always a local record of what went in.
- **`ai-work/chatgpt/`, `ai-work/claude/`, `ai-work/gemini/`** — files an AI
  makes that you download from its pane are grabbed automatically into that
  AI's folder (no save dialog).

Open it any time from **Tools → Open Output Folder**.
